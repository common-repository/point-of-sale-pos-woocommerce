...
trait ProductResponse
{
	...
	public function prepare_stock_quantity(
		...
		$response->data = (new ProductInventory(
			wc_get_product($response->data['id']),
			new Station(intval(wp_unslash($request['station_id'])))
		))->apply_data_to_response($response->data);
	...
	protected function prepare_object_for_database($request, $creating = false)
	{
		if (
			...
			$inventory = new ProductInventory(
				wc_get_product(intval(wp_unslash($request['id']))),
				new Station(intval(wp_unslash($request['station_id'])))
			);

			if (!$inventory->is_base_location()) {
				$inventory->update_location_quantity(intval(wp_unslash($request['stock_quantity'])));
				unset($request['stock_quantity']);
			}
		}
