...
class API
{
	...
	public function register_rest_routes(): void
	{
		...
		if (Frontend::checkUserAssign()) {
			...
			if (Admin\Woocommerce\GiftCards::is_plugin_active()) {
				(new API\GiftCards())->register_routes();
			}

			if (Admin\Woocommerce\PointsRewards::is_plugin_active()) {
				(new API\PointsRewards())->register_routes();
			}
		} else {
	...
	public static function is_pos(): bool
	{
		return isset($_SERVER['HTTP_X_POS']) && $_SERVER['HTTP_X_POS'];
	}

	public function rewrite_rest_routes(): void
	{
	...
	private function authenticate_cloud_app_user(): void
	{
		if (
			!(
