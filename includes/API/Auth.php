...
class Auth extends WP_REST_Controller
{
	...
	public function validate(WP_REST_Request $request)
	{
		...
		foreach ($this->get_required_params_to_validate() as $required_param) {
			if (empty($params[$required_param])) {
				return new WP_Error(
					...
					sprintf(__('Error: Missing parameter %s', 'zpos-wp-api'), $required_param),
					['status' => 400]
				);
			}
		}
		...
		if (!filter_var($params['user_email'], FILTER_VALIDATE_EMAIL)) {
			return new WP_Error('invalid_user_email', __('Error: Invalid user_email', 'zpos-wp-api'), [
				'status' => 400,
			]);
		}
		...
		if (empty($key_data)) {
			return new WP_Error(
				...
				__('Error: Invalid consumer_key', 'zpos-wp-api'),
				[
					'status' => 400,
				]
			);
		...
		if ('read_write' !== $key_data['permissions']) {
			return new WP_Error(
				...
				sprintf(__('Error: %s requires read_write permission', 'zpos-wp-api'), CLOUD_APP_NAME),
				['status' => 400]
			);
		}
		...
		if (!$key_owner || empty($key_owner->user_email)) {
			return new WP_Error('invalid_key_owner', __('Error: Invalid key owner', 'zpos-wp-api'), [
				'status' => 500,
			]);
		}
		...
		if (!$is_admin && !$is_shop_manager) {
			return new WP_Error(
				...
				__('Error: Key owner does not have the necessary permissions', 'zpos-wp-api'),
				['status' => 500]
			);
		}
	...
	protected function update_pos_users(WP_User $key_owner, string $user_email)
	{
		if (empty($key_owner->user_login)) {
			return new WP_Error(
				...
				__('Error: Key owner does not have a login', 'zpos-wp-api'),
				['status' => 500]
			);
		}
		...
		if ($user_email === $key_owner->user_email) {
			...
			if (empty($cloud_user->user_login)) {
				return new WP_Error(
					...
					sprintf(__('Error: %s user does not have a login', 'zpos-wp-api'), CLOUD_APP_NAME),
					['status' => 500]
				);
			}
	...
	protected function insert_cloud_user(string $user_email)
	{
		...
		$user_id = wp_insert_user([
			...
			'role' => 'shop_manager',
			'description' => sprintf(__('User from the %s', 'zpos-wp-api'), CLOUD_APP_NAME),
		]);

		if ($user_id instanceof WP_Error) {
			return new WP_Error(
				'cloud_user_has_not_been_created',
				sprintf(__('Error: %s', 'zpos-wp-api'), $user_id->get_error_message()),
				['status' => 500]
			);
		}
