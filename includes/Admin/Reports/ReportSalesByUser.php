...
class ReportSalesByUser extends \WC_Admin_Report
{
	...
	private function query_report_data()
	{
		...
		$this->report_data->total_tax = wc_format_decimal(
			array_sum(wp_list_pluck($this->report_data->orders, 'total_tax')) -
				$this->report_data->total_tax_refunded,
			2
		);
		$this->report_data->total_shipping = wc_format_decimal(
			array_sum(wp_list_pluck($this->report_data->orders, 'total_shipping')) -
				$this->report_data->total_shipping_refunded,
			2
		);
		$this->report_data->total_shipping_tax = wc_format_decimal(
			array_sum(wp_list_pluck($this->report_data->orders, 'total_shipping_tax')) -
				$this->report_data->total_shipping_tax_refunded,
			2
		);
		// Total the refunds and sales amounts. Sales subract refunds.
		$this->report_data->total_sales = wc_format_decimal(
			array_sum(wp_list_pluck($this->report_data->orders, 'total_sales')) -
				$this->report_data->total_refunds,
			2
		);
		$this->report_data->net_sales = wc_format_decimal(
			$this->report_data->total_sales -
				$this->report_data->total_shipping -
				$this->report_data->total_tax -
				$this->report_data->total_shipping_tax,
			2
		);
		// Calculate average based on net
		$this->report_data->average_sales = wc_format_decimal(
			$this->report_data->net_sales / ($this->chart_interval + 1),
			2
		);
		$this->report_data->average_total_sales = wc_format_decimal(
			$this->report_data->total_sales / ($this->chart_interval + 1),
			2
		);
		...
		$this->report_data->total_coupons = number_format(
			array_sum(wp_list_pluck($this->report_data->coupons, 'discount_amount')),
			2,
			'.',
			''
		);
		...
		// Item counts
		$this->report_data->total_items =
			absint(array_sum(wp_list_pluck($this->report_data->order_items, 'order_item_count'))) -
	...
	public function get_chart_legend()
	{
		...
		$legend[] = [
			'title' =>
				...
				' (' .
				sprintf(
					_n('%d item', '%d items', $this->report_data->refunded_order_items, 'woocommerce'),
					$this->report_data->refunded_order_items
				) .
				')',
	...
	public function get_export_button()
	{
		...
		<a
			href="#"
			download="report-<?php echo esc_attr($current_range); ?>-<?php echo date_i18n(
	'Y-m-d',
	...
	public function get_main_chart()
	{
		...
		$data = [
			...
			'order_amounts' => $this->prepare_chart_data(
				$this->report_data->orders,
				'post_date',
				'total_sales',
				$this->chart_interval,
				$this->start_date,
			...
			'coupon_amounts' => $this->prepare_chart_data(
				$this->report_data->coupons,
				'post_date',
				'discount_amount',
				$this->chart_interval,
				$this->start_date,
			...
			'shipping_amounts' => $this->prepare_chart_data(
				$this->report_data->orders,
				'post_date',
				'total_shipping',
				$this->chart_interval,
			...
			'refund_amounts' => $this->prepare_chart_data(
				$this->report_data->partial_refunds,
				'post_date',
				'total_refund',
				$this->chart_interval,
				$this->start_date,
			...
			'shipping_tax_amounts' => $this->prepare_chart_data(
				$this->report_data->orders,
				'post_date',
				'total_shipping_tax',
				$this->chart_interval,
			...
			'tax_amounts' => $this->prepare_chart_data(
				$this->report_data->orders,
				'post_date',
				'total_tax',
				$this->chart_interval,
		...
		foreach ($data['order_amounts'] as $order_amount_key => $order_amount_value) {
			...
			$data['gross_order_amounts'][$order_amount_key][1] -=
		...
		$chart_data = json_encode([
			...
			'order_amounts' => array_map(
				[$this, 'round_chart_totals'],
				array_values($data['order_amounts'])
			),
			'gross_order_amounts' => array_map(
				[$this, 'round_chart_totals'],
				array_values($data['gross_order_amounts'])
			),
			'net_order_amounts' => array_map(
				[$this, 'round_chart_totals'],
				array_values($data['net_order_amounts'])
			),
			'shipping_amounts' => array_map(
				[$this, 'round_chart_totals'],
				array_values($data['shipping_amounts'])
			),
			'coupon_amounts' => array_map(
				[$this, 'round_chart_totals'],
				array_values($data['coupon_amounts'])
			),
			'refund_amounts' => array_map(
				[$this, 'round_chart_totals'],
	...
	->average_total_sales; ?> ], [ <?php echo max(
 	array_keys($data['order_amounts'])
 ); ?>, <?php echo $this->report_data->average_total_sales; ?> ]],
							yaxis: 2,
						...
						},
						{
							...
							data: [[ <?php echo min(array_keys($data['order_amounts'])); ?>, <?php echo $this->report_data
	->average_sales; ?> ], [ <?php echo max(array_keys($data['order_amounts'])); ?>, <?php echo $this
	->report_data->average_sales; ?> ]],
							yaxis: 2,
