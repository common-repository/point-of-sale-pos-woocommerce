...
abstract class Page
{
	...
	public function __construct($slug, $parent)
	{
		...
		add_action('admin_footer', [$this, 'redirectAfterOptionsSave'], 100000);
	}

	public function redirectAfterOptionsSave()
	{
		if (isset($_GET['pos_update_page']) && $_GET['pos_update_page']) {
			$redirect_url = admin_url('admin.php?page=' . $_GET['pos_update_page']);
	...
	public function render()
	{
		...
		<form action="<?= admin_url('options.php?pos_update_page=' . $this->slug) ?>" method="post">
			<div id="root"></div>
			<script>
