...
class Post
{
	...
	public function __construct()
	{
		...
		add_action('before_delete_post', [$this, 'deletePost'], 10, 2);
	}

	public function deletePost($post_id, $post): void
	{
		try {
			...
			foreach (['test', 'live'] as $mode) {
				$station = new Station($post_id);
				$location_id = $station->getLocationId($mode);

				if ($location_id) {
					$client = new StripeConnectRestClient($mode);
					$client->deleteRequest('locations/' . $location_id);
				}
			}
		} catch (\Exception $e) {
			wc_get_logger()->error('Failed to delete location for station', [
				'source' => PLUGIN_NAME,
				'post_id' => $post_id,
				'exception' => $e,
			]);
