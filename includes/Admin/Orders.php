...
class Orders
{
	public function __construct()
	{
		...
		add_action('woocommerce_payment_gateways', [$this, 'add_pos_gateways'], PHP_INT_MAX);
		...
		if (static::is_custom_table_enabled()) {
	...
	public static function is_list_screen(): bool
	{
		...
		return static::is_custom_table_enabled()
			? OrderUtil::is_order_list_table_screen()
			: 'edit.php' === $pagenow &&
	...
	public static function is_edit_screen(): bool
	{
		...
		return static::is_custom_table_enabled()
			? OrderUtil::is_order_edit_screen()
			: 'post.php' === $pagenow &&
					isset($_GET['post']) &&
					wc_get_order(intval(wp_unslash($_GET['post'])));
	}

	public static function is_custom_table_enabled(): bool
	{
	...
	public function typeColumn($column, $order_id)
	{
		...
		$src = plugins_url($isPOS ? 'assets/admin/pos.svg' : 'assets/admin/web.svg', PLUGIN_ROOT_FILE);
		$alt = $isPOS ? __('POS order', 'zpos-wp-api') : __('Web order', 'zpos-wp-api');
	...
	public function index_order_views($views)
	{
		$order_type = isset($_GET['order_type'])
			? sanitize_text_field(wp_unslash($_GET['order_type']))
			: null;
	...
	protected function get_formatted_price(WC_Order $order, $price): string
	{
	...
	public function add_pos_gateways(array $gateways): array
	{
		global $pagenow;
		...
		$is_order_edit_screen =
			...
			isset($_GET['id'])
				? boolval(wc_get_order(sanitize_text_field(wp_unslash($_GET['id']))))
				: 'post.php' === $pagenow &&
					isset($_GET['post']) &&
					wc_get_order(sanitize_text_field(wp_unslash($_GET['post'])));

		if (!$is_order_edit_screen) {
			return $gateways;
		}

		return array_merge($gateways, Model\Gateway::posGateways());
	}
