...
class SplitPayment extends Base
{
	...
	/**
	 * @param mixed $raw_post_data
	 */
	public static function is_split_payment($raw_post_data = []): bool
	{
	...
	public function process_payment($order_id): array
	{
		...
		$gateways = Model\Gateway::get_available_gateways();
		$tendered = $this->get_split_payment_tendered($split_payments, $order, $gateways);
		$order_total = $order->get_total();
		$tendered_int = (int) round($tendered * 100);
		$order_total_int = (int) round($order_total * 100);
		$is_amount_match = in_array(Cash::getID(), $split_payments, true)
			? $tendered_int >= $order_total_int
			: $tendered_int === $order_total_int;

		if (!$is_amount_match) {
			$response_data = [
				'result' => 'failed',
				'message' => __('Oops! The amount does not match.', 'zpos-wp-api'),
			];
		...
		if ('success' === $response_data['result']) {
			...
			$order->update_status(Model\Gateway::getGatewayOrderStatus($this->id));
		} else {
	...
	public function get_split_payment_tendered(
		...
		return array_reduce(
			$split_payments,
			function (float $result, string $split_payment) use ($order, $gateways) {
				$key = str_replace('_', '-', $split_payment);
				$amount_tendered = (float) $order->get_meta($key . '-tendered');
				$is_gift_card_addon =
					'pos_gift_card' === $split_payment && Woocommerce\GiftCards::is_plugin_active();

				if (
					empty($gateways[$split_payment]) ||
					!is_numeric($amount_tendered) ||
					$is_gift_card_addon
				) {
					return $result;
				}
	...
	private function get_massage_failed(array $failed_payments, array $gateways): string
	{
		...
		$message =
			1 === $failed_payments_count
				? __('Oops! Payment method %s has failed processing.', 'zpos-wp-api')
				: __('Oops! Following payment methods have failed processing: %s.', 'zpos-wp-api');
	...
	private function get_base_failed_response(): array
	{
		return [
			'result' => 'failed',
			'message' => __('Oops! Something went wrong.', 'zpos-wp-api'),
		];
	}
}
